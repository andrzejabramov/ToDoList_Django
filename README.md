...
{% endblock %}
```
В данном шаблоне таже присутствует блок ссылки на базовый шаблон (extends)
Значение текстовое, подставляемое в блок title базового шаблона
Блок body начинается с заголовков <h2> и <h3>
Затем следует защищенная тегом {% csrf_token %} форма для добавления задания, после отработки кнопки ADD возврат на начальную страницу с отоображением нового списка 
Форма с помощью директивы {{ form.as_p }} представляется в виде параграфа (<p>)
После отработки формы возврат на начальную страницу с обновленным списком (добавлена строка)
...
{% endblock %}
```
Данный шаблон работает по аналогии с предыдущим, только на удаление записи задания 
